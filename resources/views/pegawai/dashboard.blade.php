...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="animate-slide-up pt-4">
        <h1 class="text-xl md:text-2xl font-bold text-sihati-ink tracking-tight">{{ $greeting }}, {{ $user->name }}!</h1>
        <p class="text-sihati-slate mt-1 text-sm">{{ $user->bidang?->nama_bidang ?? 'Laporkan kendala perangkat Anda disini' }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div class="stat-card animate-slide-up delay-100">
            <div class="flex items-center gap-4">
                ...
                <div><p class="text-2xl font-bold text-sihati-ink">{{ $totalAduan }}</p><p class="text-sm text-sihati-slate">Total Aduan</p></div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 animate-slide-up delay-100">
        <div class="bg-sihati-canvas rounded-xl border border-sihati-hairline shadow-soft p-5 md:p-7 card-hover flex flex-col">
        ...
        <div class="bg-sihati-canvas rounded-xl border border-sihati-hairline shadow-soft p-5 md:p-7 card-hover flex flex-col">
            <div class="flex flex-col sm:flex-row items-stretch gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <h2 class="text-lg md:text-xl font-bold text-sihati-ink">Detail Aduan</h2>
                    <p class="text-sihati-slate text-sm md:text-base mt-1 leading-relaxed">Lihat detail aduan yang sudah Anda kirimkan.</p>
                </div>
            </div>
            <div class="mt-auto flex justify-end pt-4">
    ...
    @php
        $aduanPaginator = $aduanTerbaru;
        $pagCurrent = $aduanPaginator->currentPage();
        $pagLast    = $aduanPaginator->lastPage();
        $pagMaxVis  = 5;
        $pagHalf    = floor(($pagMaxVis - 1) / 2);
        $pagStart   = max($pagCurrent - $pagHalf, 1);
        $pagEnd     = min($pagStart + $pagMaxVis - 1, $pagLast);
        if ($pagEnd - $pagStart + 1 < $pagMaxVis) {
            $pagStart = max($pagEnd - $pagMaxVis + 1, 1);
        }
        $pagPrevUrl = $aduanPaginator->previousPageUrl();
        $pagNextUrl = $aduanPaginator->nextPageUrl();
        $pagHas     = $aduanPaginator->hasPages();
    @endphp

    <div class="bg-sihati-canvas rounded-xl border border-sihati-hairline shadow-soft">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-5 py-4 border-b border-sihati-hairline">
            <h2 class="text-lg font-semibold text-sihati-ink tracking-tight">Aduan Hari Ini</h2>
            @if($pagHas)
            <nav class="flex items-center gap-1" aria-label="Pagination">
                {{-- Previous --}}
                <a href="{{ $pagPrevUrl ?? '#' }}"
                   class="inline-flex h-7 w-7 items-center justify-center rounded-md text-xs transition {{ $pagPrevUrl ? 'text-sihati-slate hover:bg-sihati-surface hover:text-sihati-ink' : 'pointer-events-none text-sihati-hairline-strong' }}">
                ...
                @for ($i = $pagStart; $i <= $pagEnd; $i++)
                    @if ($i === $pagCurrent)
                        <span class="inline-flex h-7 min-w-[28px] items-center justify-center rounded-md bg-sihati-primary px-1.5 text-xs font-semibold text-white">{{ $i }}</span>
                    @else
                        <a href="{{ $aduanPaginator->url($i) }}"
                           class="inline-flex h-7 min-w-[28px] items-center justify-center rounded-md px-1.5 text-xs text-sihati-slate transition hover:bg-sihati-surface hover:text-sihati-ink">{{ $i }}</a>
                    @endif
                @endfor
                {{-- Next --}}
                <a href="{{ $pagNextUrl ?? '#' }}"
                   class="inline-flex h-7 w-7 items-center justify-center rounded-md text-xs transition {{ $pagNextUrl ? 'text-sihati-slate hover:bg-sihati-surface hover:text-sihati-ink' : 'pointer-events-none text-sihati-hairline-strong' }}">
                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </a>
            </nav>
            @endif
        </div>
        @if($aduanTerbaru->total() > 0)
        ...
        <div class="px-5 py-8 text-center">
            <p class="text-sm text-sihati-slate">Belum ada aduan hari ini.</p>
        </div>
        @endif
